...
local ShindoBoosterV1Folder = "ShindoBoosterV1"
local ConfigurationFolder = ShindoBoosterV1Folder.."/Configurations"
local ConfigurationExtension = ".rfld"



local ShindoBoosterV1Library = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", 
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across ShindoBoosterV1
...
local ShindoBoosterV1 = game:GetObjects("rbxassetid://10804731440")[1]

ShindoBoosterV1.Enabled = false


if gethui then
	ShindoBoosterV1.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(ShindoBoosterV1)
	ShindoBoosterV1.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	ShindoBoosterV1.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	ShindoBoosterV1.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ShindoBoosterV1.Name and Interface ~= ShindoBoosterV1 then
			Interface.Enabled = false
			Interface.Name = "ShindoBoosterV1-Old"
		end
	end
else
...
local Main = ShindoBoosterV1.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

ShindoBoosterV1.DisplayOrder = 100
...
local Notifications = ShindoBoosterV1.Notifications

local SelectedTheme = ShindoBoosterV1Library.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = ShindoBoosterV1Library.Theme[ThemeName]
	for _, obj in ipairs(ShindoBoosterV1:GetDescendants()) do
	...
	ShindoBoosterV1.Main.BackgroundColor3 = SelectedTheme.Background
	ShindoBoosterV1.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ShindoBoosterV1.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ShindoBoosterV1.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	ShindoBoosterV1.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ShindoBoosterV1.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ShindoBoosterV1.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if ShindoBoosterV1Library.Flags[FlagName] then
			spawn(function() 
				if ShindoBoosterV1Library.Flags[FlagName].Type == "ColorPicker" then
					ShindoBoosterV1Library.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if ShindoBoosterV1Library.Flags[FlagName].CurrentValue or ShindoBoosterV1Library.Flags[FlagName].CurrentKeybind or ShindoBoosterV1Library.Flags[FlagName].CurrentOption or ShindoBoosterV1Library.Flags[FlagName].Color ~= FlagValue then ShindoBoosterV1Library.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			ShindoBoosterV1Library:Notify({Title = "Flag Error", Content = "ShindoBoosterV1 was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(ShindoBoosterV1Library.Flags) do
...
function ShindoBoosterV1Library:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= ShindoBoosterV1Library.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("ShindoBoosterV1 | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if ShindoBoosterV1.Name == "ShindoBoosterV1" then
...
function Hide()
	Debounce = true
	ShindoBoosterV1Library:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function ShindoBoosterV1Library:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "ShindoBoosterV1 Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "ShindoBoosterV1 Interface Suite" then
		LoadingFrame.Version.Text = "ShindoBoosterV1 UI"
	end
	...
	pcall(function()
		...
		if not isfolder(ShindoBoosterV1Folder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(ShindoBoosterV1Folder.."/Discord Invites") then
			makefolder(ShindoBoosterV1Folder.."/Discord Invites")
		end
		if not isfile(ShindoBoosterV1Folder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ShindoBoosterV1Folder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ShindoBoosterV1 RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(ShindoBoosterV1Folder.."/Key System") then
			makefolder(ShindoBoosterV1Folder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("ShindoBoosterV1 | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(ShindoBoosterV1Folder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(ShindoBoosterV1Folder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
		...
		if not Passthrough then
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ShindoBoosterV1Folder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						ShindoBoosterV1Library:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
			...
			KeyMain.Hide.MouseButton1Click:Connect(function()
				...
				TweenService:Create(KeyMain.Hide, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {ImageTransparency = 1}):Play()
				wait(0.51)
				ShindoBoosterV1Library:Destroy()
				KeyUI:Destroy()
			end)
		else
			Passthrough = true
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ShindoBoosterV1 | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ShindoBoosterV1Library.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ShindoBoosterV1 | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("ShindoBoosterV1 | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ShindoBoosterV1Library.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ShindoBoosterV1 | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ShindoBoosterV1Library.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ShindoBoosterV1 | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					ShindoBoosterV1Library.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ShindoBoosterV1 | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ShindoBoosterV1Library.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function ShindoBoosterV1Library:Destroy()
	ShindoBoosterV1:Destroy()
end
...
function ShindoBoosterV1Library:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				ShindoBoosterV1Library:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, ShindoBoosterV1Library.LoadConfiguration, ShindoBoosterV1Library)

return ShindoBoosterV1Library